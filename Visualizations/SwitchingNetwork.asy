...
//
// A leaf draws a horizontal wire with a gap in the middle, a diagonal tick across the gap (the
// open-switch symbol), and the variable's label above it (barred if negated). An AND draws its two
// children side by side, each vertically offset so their connection points land on the same line —
// the higher of the two center_y values, so no offset is ever negative. An OR draws its two
// children stacked (the first on top) with a gap between them, padding whichever is narrower with a
// plain wire extension (split evenly on both ends) so both span the same width, then connects their
// two connection points with a vertical rail on each side.
//
// Inputs:
...
void draw_node(picture pic, ExprNode node, real scale, real x0, real y0) {
    if (node.kind == EXPR_LEAF) {
        ...
        NetworkLayout b = measure_node(node.children[1], scale);
        real max_cy = max(a.center_y, b.center_y);
        draw_node(pic, node.children[0], scale, x0, y0 + (max_cy - a.center_y));
        draw_node(pic, node.children[1], scale, x0 + a.width, y0 + (max_cy - b.center_y));
    } else {
        NetworkLayout a = measure_node(node.children[0], scale);
        NetworkLayout b = measure_node(node.children[1], scale);
        ...
        draw_node(pic, node.children[0], scale, x0 + a_pad, a_y0);
        draw_node(pic, node.children[1], scale, x0 + b_pad, b_y0);
...
struct SwitchingNetwork {
    ...
    picture render(real width, real height, real unit) {
        ...
        draw_node(pic, this._root, net_scale, offset_x + lead, offset_y);
