...
// Test file to visualize text_large typography spacing
...
real spacing = 1.0 * 2.0 / 3.0 * 0.9 * 0.9 * 0.9 * 0.9;  // Starting with text_normal spacing
...
for (int row = 0; row < rows; ++row) {
    for (int col = 0; col < cols; ++col) {
        if (char_index < total_chars) {
            ...
            label(ch, (cell_x + spacing / 2, cell_y - spacing / 2), p=text_large.p, align=Center);
