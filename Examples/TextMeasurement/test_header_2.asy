...
// Test file to visualize header_2 typography spacing
...
real spacing = 1.0 * 2.0 / 3.0 * 0.9 * 0.9 * 0.9 * 0.9 * 2.0 * 0.8 * 0.9;  // Doubled from text_normal, then reduced by 20%, then 10%
...
for (int row = 0; row < rows; ++row) {
    for (int col = 0; col < cols; ++col) {
        if (char_index < total_chars) {
            ...
            label(ch, (cell_x + spacing / 2, cell_y - spacing / 2), p=header_2.p, align=Center);
